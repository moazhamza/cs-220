 ...
 *
 * @param request_vpn The virtual page number requested.
...
pfn_t pagefault_handler(vpn_t request_vpn, int write){
	 ...
	 */
    if(victim_pcb){
    	if(IS_SET(victim_pcb->pagetable[victim_vpn].flags, DIRTY)) page_to_disk(victim_pfn, victim_vpn, victim_pcb->pid);
	...
	else CLEAR_BIT(current_pagetable[request_vpn].flags, DIRTY);
